...
+temp(T) : heating(true) & not window_state("closed")
    <- .print("Warning: Heating active with window not closed. Closing window to prevent heat loss.");
       close.
...
+window_state(State) : State \== "closed" & heating(true)
    <- .print("Warning: Window state changed to ", State, " while heating. Closing window.");
       close.