...
+temp(T) : heating(true) & not window_state("closed")
    <- .print("Warning: Heating is on but window is not closed. Closing window to prevent heat loss.");
       close.
...
+window_state(State) : State \== "closed" & heating(true)
    <- .print("Window state changed to ", State, " while heating is active. Closing window.");
       close.