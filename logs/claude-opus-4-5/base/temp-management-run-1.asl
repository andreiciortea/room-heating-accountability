...
+temp(T) : heating(true) & window_state("tilted")
    <- .print("Heating active but window is tilted. Closing window to prevent heat loss.");
       close.

+window_state("tilted") : heating(true)
    <- .print("Window opened while heating is active. Closing window to prevent heat loss.");
       close.
